...
## 1. Contracts
- [ ] 1.1 `lib/weather/types.ts` — `DailyForecast`, `Forecast`, map result union.

## 2. Failing tests first (red)
- [ ] 2.1 `lib/weather/map.test.ts`: `mapForecast` on a well-formed daily+hourly
...
- [ ] 2.2 `lib/weather/map.test.ts` (failures): empty/missing `daily`, missing
        hourly temps, null sunrise/sunset, malformed/unparseable → `{ ok:false }`,
        NEVER throws. `@trace FR-FORECAST-01, NFR-OBS-01`.
- [ ] 2.3 `lib/weather/code.test.ts`: `weatherCodeToCondition` maps representative
        WMO codes to an icon key + Ukrainian condition name. `@trace FR-FORECAST-02`.
- [ ] 2.4 `lib/weather/weekday.test.ts`: `ukWeekday(localDateStr)` returns the
        Ukrainian weekday from the LOCAL date string (timezone-invariant, never
        UTC slice). `@trace FR-FORECAST-02`.
- [ ] 2.5 Eval case `evals/cases/forecast.eval.ts`: forecast-error copy + condition
        names — calm Ukrainian, no "!"/emoji. `@trace FR-FORECAST-01, BC-BRAND-01`.
- [ ] 2.6 Run `npm run test:run` — confirm RED.

## 3. Implementation (green)
- [ ] 3.1 `lib/weather/*` — pure types + `mapForecast` (total), `weatherCodeToCondition`,
        `ukWeekday`. No next/react/DOM.
- [ ] 3.2 Forecast fetch helper: `fetchForecast(lat,lon)` server-side, wrapped in
...
- [ ] 3.3 `components/forecast/DayCard.tsx` (server) — weekday, hi/lo °C,
        weather-code icon w/ UA alt, precip %, wind, `ComfortBadge`.
- [ ] 3.4 `components/forecast/HourlyChart.tsx` (`"use client"`, Recharts) +
        `HourlyChartLazy.tsx` (`"use client"` wrapper using
        `dynamic(() => import('./HourlyChart'), { ssr:false })`); UA accessible name.
- [ ] 3.5 `components/forecast/SunTimes.tsx`, `ForecastError.tsx` (calm inline +
        retry), and the weekend comfort highlight (uses `weekendComfort`).
- [ ] 3.6 Wire the location view in `app/page.tsx`: validate coords → fetch → map →
...
- [ ] 4.3 `npx tsc --noEmit`
- [ ] 4.4 `npm run build`
- [ ] 4.5 `npx openspec validate add-forecast --strict` + `--all --strict`
- [ ] 4.6 Run review-slice; fix confirmed findings; re-run 4.1–4.5.
- [ ] 4.7 Update `docs/current-state.md`.
- [ ] 4.8 Real fetch smoke: `fetchForecast` for a known city returns a mapped
