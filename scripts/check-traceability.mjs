...
// Requirement ids in this project use the PRD's semantic scheme
// `PREFIX-CATEGORY-NN` (e.g. FR-SEARCH-01, NFR-A11Y-01, TC-STACK-01) as well as
// the Factory's bare `PREFIX-NN` (e.g. BUG-3). The category segment is optional
// and may itself contain digits (A11Y, I18N). This recognizes both forms.
const ID_RE = /\b(?:FR|NFR|TC|BC|BUG)-(?:[A-Z0-9]+-)?\d+\b/g;
const idsIn = (text) => [...new Set(text.match(ID_RE) ?? [])];
...
for (const dir of PATHS.testDirs) {
  for (const file of walk(dir, isTestFile)) {
    ...
    for (const m of text.matchAll(/@trace\s+((?:FR|NFR|TC|BC|BUG)-(?:[A-Z0-9]+-)?\d+(?:\s*,\s*(?:FR|NFR|TC|BC|BUG)-(?:[A-Z0-9]+-)?\d+)*)/g)) {
